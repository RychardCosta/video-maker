...
async function robot() {
  ...
  async function authenticateWithOAuth() {
    ...
    async function createOAuthClient() {
      const credentials = require('../credencials/youtube.json')
...
async function convertToMp4(){
  convert = await new Promise((resolve, reject) => {
    const destinationFilePath = `${rootPath}\\content\\output.mov`
    const destinationFilePathConverted = `${rootPath}\\content\\output.mp4`

    console.log('> [youtube-robot] Converting to MP4 for best performace')
    ...
    .on("error", err => {
      console.error(`> [video-robot] Erro encontrado ao tentar converter o vídeo: ${err}`)
      })
      .on('progress', progress => {
          console.log(`Percent complete: ${progress.percentComplete}%s`)
          if(progress.percentComplete === 100) {
            console.log('> [youtube-robot] Converted')
            resolve()
